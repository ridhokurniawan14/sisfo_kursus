        ...
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <div class="card card-warning">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form method="POST" action="/pengumuman/{{ $cari->id }}" enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="card-body">
                  <div class="row">
                    <div class="col-sm-3">
                      <div class="form-group">
                        ...
                        <select autofocus name="jenis" id="jenis" class="form-control @error('jenis') is-invalid @enderror" required>
                          ...
                          <option value="pemberitahuan" {{ old('jenis', ($cari->jenis)) == 'pemberitahuan' ? 'selected' : '' }}>Pemberitahuan</option>
                          <option value="libur kursus" {{ old('jenis', ($cari->jenis)) == 'libur kursus' ? 'selected' : '' }}>Libur Kursus</option>
                          <option value="lowongan pekerjaan" {{ old('jenis', ($cari->jenis)) == 'lowongan pekerjaan' ? 'selected' : '' }}>Lowongan Pekerjaan</option>
                          <option value="lomba" {{ old('jenis', ($cari->jenis)) == 'lomba' ? 'selected' : '' }}>Lomba</option>
                        </select>
                        @error('jenis')
                    ...
                    <div class="col-sm-5">
                      <div class="form-group">
                        ...
                        <input autocomplete="off" value="{{ old('judul', (ucwords($cari->judul))) }}" required type="text" name="judul" class="form-control @error('judul') is-invalid @enderror" id="judul" placeholder="Misal : Libur Hari Raya">
                        @error('judul')
                    ...
                    <div class="col-sm-4">
                      <div class="form-group">
                        ...
                        <select name="untuk" id="untuk" class="form-control @error('untuk') is-invalid @enderror" required>
                          ...
                          <option value="semua" {{ old('untuk', ($cari->untuk)) == 'semua' ? 'selected' : '' }}>Semua</option>
                          <option value="peserta didik" {{ old('untuk', ($cari->untuk)) == 'peserta didik' ? 'selected' : '' }}>Peserta Didik</option>
                          <option value="pendidik" {{ old('untuk', ($cari->untuk)) == 'pendidik' ? 'selected' : '' }}>Pendidik</option>
                        </select>
                        @error('untuk')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                  ...
                  <div class="form-group">
                    ...
                    <textarea name="ket" autocomplete="off" class="form-control @error('ket') is-invalid @enderror" rows="3" id="ket" placeholder="Masukkan Deskripsi Pengumuman secara detail (Hari, tgl. Jam dst.)" required>{{ old('ket', (ucfirst($cari->ket))) }}</textarea>
                    @error('ket')
                      <div class="invalid-feedback">
                        {{ $message }}
                      </div>
                ...
                <div class="card-footer">
                  ...
                  <a href="/pengumuman"><button type="button" class="btn btn-secondary">Kembali</button></a>
                </div>
              </form>
            </div>
            <!-- /.card -->
